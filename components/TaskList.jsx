...
export default function TaskList({ tasks, setTasks, refreshTasks }) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await fetch("http://localhost:5001/users", { credentials: "include" });
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5001/tasks/${editingTask}`, {
        method: "PUT",
        credentials: "include",
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5001/tasks/${deletingTask}`, {
        method: "DELETE",
        credentials: "include",
      });
